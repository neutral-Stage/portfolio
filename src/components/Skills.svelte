...
>
  ...
  <div class="relative z-10 max-w-7xl mx-auto">
    ...
    <div class="text-center mb-20 animate-scale-in">
      <div class="inline-flex items-center gap-3 px-6 py-3 glass-light rounded-full mb-6">
        <Sparkles size={20} class="text-purple-400" />
        <span class="text-sm font-semibold text-gray-300 uppercase tracking-wider">Skills & Expertise</span>
      </div>
    ...
    <!-- Category Tabs -->
    <div class="flex flex-wrap justify-center gap-3 mb-16 animate-slide-up delay-200">
      {#each skillCategories as category, index}
        ...
        >
          {#if selectedCategory === index}
            ...
            <div class="absolute inset-0 bg-gradient-to-r {category.color} opacity-20 rounded-xl"></div>
          {/if}
          <span class="relative z-10 flex items-center gap-2">
            <svelte:component this={category.icon} size={18} class="group-hover:scale-110 transition-transform" />
            {category.title}
          </span>
          {#if selectedCategory !== index}
            <div class="absolute inset-0 glass-light opacity-0 group-hover:opacity-100 rounded-xl transition-opacity"></div>
          {/if}
        </button>
      {/each}
    </div>
          ...
          <div class="animate-scale-in">
            ...
            <div class="card-glow glass-strong p-8 rounded-3xl mb-8">
              <div class="flex items-center gap-6 mb-2">
                <div class="w-16 h-16 rounded-2xl bg-gradient-to-br {category.color} flex items-center justify-center">
                  <svelte:component this={category.icon} size={32} class="text-white" />
                </div>
                <div>
                  <h3 class="text-3xl font-black text-white mb-1">{category.title}</h3>
                  <p class="text-gray-400">Expert level proficiency in these technologies</p>
                </div>
              </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
              {#each category.skills as skill, skillIndex}
                <div 
                  class="card-3d card-shine glass p-6 rounded-2xl group hover:scale-[1.02] transition-all duration-300 animate-slide-up delay-{skillIndex * 100}"
                >
                  <div class="flex justify-between items-start mb-4">
                    <div>
                      <h4 class="text-lg font-bold text-white mb-1">{skill.name}</h4>
                    ...
                    <div class="relative w-16 h-16">
                      ...
                      <svg class="transform -rotate-90 w-16 h-16" viewBox="0 0 64 64">
                        ...
                        <circle
                          ...
                          stroke-dashoffset={175.93 - (175.93 * (isVisible ? skill.level : 0)) / 100}
                          class="transition-all duration-1000 ease-out"
                          style="transition-delay: {skillIndex * 100}ms;"
                        />
                        <defs>
                          <linearGradient id="gradient-{catIndex}-{skillIndex}" x1="0%" y1="0%" x2="100%" y2="100%">
                            <stop offset="0%" class="text-purple-500" stop-color="currentColor" />
                            <stop offset="100%" class="text-pink-500" stop-color="currentColor" />
                          </linearGradient>
                        </defs>
                      </svg>
                      <!-- Percentage Text -->
                      <div class="absolute inset-0 flex items-center justify-center">
                        <span class="text-sm font-black gradient-text">{skill.level}%</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="space-y-2">
                    <div class="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden">
                      <div
                        ...
                        style="width: {isVisible ? skill.level : 0}%; transition-delay: {skillIndex * 100}ms;"
                      ></div>
                    </div>
                  </div>
                </div>
    ...
    <div class="mt-24 text-center animate-slide-up delay-400">
      ...
      <div class="flex flex-wrap justify-center gap-3 max-w-6xl mx-auto">
        {#each ["Tailwind CSS", "Styled Components", "GraphQL", "Redis", "Elasticsearch", "Jest", "Cypress", "Storybook", "Webpack", "Babel", "ESLint", "Prettier", "Sass", "WebGL", "Three.js", "D3.js", "Chart.js", "Socket.io", "Express.js", "FastAPI", "Prisma", "TypeORM", "JWT", "OAuth", "Stripe", "Firebase", "Supabase", "Vercel", "Netlify", "Heroku", "DigitalOcean"] as tech, index}
          <span
            class="glass-light px-5 py-2.5 rounded-full text-sm font-semibold text-gray-300 hover:text-purple-300 hover:scale-110 transition-all duration-300 cursor-default animate-bounce-in delay-{(index * 30)}"
          >
            {tech}
          </span>
        {/each}
